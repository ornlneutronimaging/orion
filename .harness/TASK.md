...
- [?] R10: "make a new release" — changes land on main via a release PR with CI green (never tag directly on main). PR opened: `gh pr create` -> https://github.com/ornlneutronimaging/orion/pull/60. BLOCKED ON USER: the merge is gated on R12. `.claude/skills/release-orion/SKILL.md` states "Never skip the manual test gate — CI does not exercise the launcher, wizard, or platform packaging." I cannot merge until you confirm the smoke test passed.
