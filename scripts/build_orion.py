...
# The VS Code base Orion ships. Pinned so that local, CI, and release builds all package
# the same base and the version claimed in CHANGELOG.md is the one users actually get.
# Override with ORION_VSCODE_VERSION=<x.y.z>, or =latest to track upstream stable.
VSCODE_VERSION = "1.127.0"
...
def get_vscode_version():
    override = os.environ.get("ORION_VSCODE_VERSION")

    if override and override != "latest":
        print(f"Using VS Code version from ORION_VSCODE_VERSION: {override}")
        return override

    if override != "latest":
        print(f"Using pinned VS Code version: {VSCODE_VERSION}")
    ...
    if not isinstance(data, list) or not data:
        raise Exception(f"Invalid API response format from {url}")
    version = data[0]
...
def is_prerelease(version_entry):
    ...
    return any(
        prop.get("key") == "Microsoft.VisualStudio.Code.PreRelease" and str(prop.get("value")).lower() == "true"
...
def get_extension_info(extension_id):
    ...
    payload = {
        ...
        # Include versions, files, version properties, asset URIs, statistics.
        # Deliberately omits IncludeLatestVersionOnly (512): that returns only the newest
        # build, which is the pre-release one whenever a publisher ships that channel.
        # We need the full history to pick the newest *release* version.
        "flags": 403,
    }
...
def install_extensions(install_dir, data_dir):
    ...
    failed = [ext for ext in extensions if not install_with_dependencies(ext)]
    if failed:
        raise Exception(
            f"Failed to install bundled extensions: {', '.join(failed)}. "
            "Refusing to package an artifact that is missing extensions users expect."
        )
