...
export class PixiService {
    ...
    public async checkAndInstall(progressCallback?: (msg: string) => void): Promise<void> {
    ...
    public async runInstall(targetDir: string, progressCallback?: (msg: string) => void): Promise<void> {
        if (progressCallback) progressCallback("Running pixi install...");

        const pixiPath = this.getPixiPath();

        return new Promise((resolve, reject) => {
            const child = cp.spawn(pixiPath, ['install'], { cwd: targetDir, shell: true });
