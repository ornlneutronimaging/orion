        ...
        if (fs.existsSync(targetDir)) {
            ...
            if (files.length > 0) {
                ...
                try {
                    ...
                    if (isRepo) {
                        console.log(`Directory ${targetDir} is already a git repository. Skipping clone.`);
                        // Proceed to checkout branch
                    } else {
                        throw new Error(`Directory ${targetDir} exists, is not empty, and is not a git repository.`);
                    }
                } catch (e) {
                    // If checkIsRepo fails, it might not be a repo, or other error.
                    // If we threw above, rethrow.
                    if (e instanceof Error && e.message.includes('exists, is not empty')) {
                        throw e;
                    }
